...
p1 <- ggplot(depth_curve_sum, aes(x = reads_number, y = mean_cov)) +
  ...
  labs(
    title = "Fig1: Reads-Coverage curve (mean-bootstrap 95% CI)",
    x = "Downsampled reads",
    y = "Genome coverage"
  ) +
  theme_base

ggsave(file.path(out_dir, "Fig1_reads_coverage.pdf"), p1, width = 6.5, height = 4.2)
...
ggsave(file.path(out_dir, "Fig2_lorenz_gini.pdf"), p2, width = 6.5, height = 7.2)
write_tsv(gini_tbl, file.path(out_dir, "Fig2_gini_by_sample.tsv"))
...
p3 <- ggplot(dist_sum, aes(x = depth, y = y_med)) +
  ...
  labs(
    title = "Fig3: Depth distribution (median with 10-90% band; depth 0-100)",
    x = "Depth",
    y = "Fraction of bases"
  ) +
  theme_base

ggsave(file.path(out_dir, "Fig3_depth_distribution_0_100.pdf"), p3, width = 6.8, height = 4.6)
...
p4 <- ggplot(chrom, aes(x = bin_mb, y = cov_frac, group = sample_id)) +
  ...
  scale_y_continuous(labels = percent, limits = c(0, 1)) +
  labs(
    title = "Fig4: 1Mb window coverage across chromosomes (one line per sample)",
...
ggsave(file.path(out_dir, "Fig4_chrom_window_coverage_all_samples.pdf"), p4, width = 18, height = 10)
ggsave(file.path(out_dir, "Fig4_chrom_window_coverage_all_samples.png"), p4, width = 18, height = 10, dpi = 300)
...
write_tsv(vaf, file.path(out_dir, "Fig5_vaf_all_mutations_including_zero.tsv"))
vaf_exc0 <- vaf %>% filter(vaf > 0)
write_tsv(vaf_exc0, file.path(out_dir, "Fig5_vaf_all_mutations_excluding_zero.tsv"))
...
plot_vaf_raw(vaf, "Fig5: Raw VAF distribution including zero", "Fig5_vaf_raw_including_zero")
plot_vaf_raw(vaf_exc0, "Fig5: Raw VAF distribution excluding zero", "Fig5_vaf_raw_excluding_zero")
